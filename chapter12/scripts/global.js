...
function prepareInternalnav() {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        //默认都是隐藏的，然后要后面手贱的点一下就出现了。
        links[i].destnation = sectionId;
...
function displayAbbreviations() {
    ...
    for (var key in defs) {
        ...
        dtitile.appendChild(dtitle_text);
    ...
    var aritcles = document.getElementsByTagName('article');
    if (aritcles.length == 0)
        return false;

    var container = aritcles[0];
...
function resetFields(whichform){
    //先判断该浏览器是否支持placeholder
    if(Modernizr.input.plceholer)
        return;
    for(var i = 0; i < whichform.elements.length; i++){
        ...
        if(element.type == 'submit')
            continue;
        //判断是否已经含有placeholder，然后创建或者取出属性值
